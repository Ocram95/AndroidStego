...
from Reassemble_apks.utils import sha256sum
import os
...
class VirusTotal:
    ...
    def __scan_apk_file(self, apk_file_path: str) -> Dict:
        ...
        report = self.get_report(sha256_hash)
        if report is None:
            raise Exception(
    ...
    def analyse_apk(self, apk_file_path: str):
        ...
        try:
            ...
            self.__logger.info(
                f"Apk {apk_file_path} scan result {self.__get_positives(report)} positives: {pformat(report)}"
            )
            report_path = os.path.join(self.__save_path, apk_file_path)
